...
All the other parameters are already available in the `configfile_DNAmapping.yaml` file or hard-coded in the snakemake file. Inspect the hard-coded parameters to make sure they work for your data. **_NOTE_**:The default of DNA_mapping is to mark duplicates but not remove them. If you wish to remove them, add the `remove_duplicates="True"` in the DNA_mapping command (if true the tag in the bams will be *_dedup* instead of *_mdup*). 
