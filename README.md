...
* effective genome size and any chromosomes to ignore bigwig for normalization (patch chromosomes) should be changed to reflect your own genomes in `peakCalling.snakefile`. This information is used in the RPGC normaliztion of bigwigs and hg38 blacklist is used to automatically filter hg38 peakcalling output files with bedtools. For hg19, the blacklist is used for normalization only, peaks are not automatically filtered (to remain backwards compatible with older Zwart lab datasets). 
