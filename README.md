...
* path to BWA indexed genome reference files should be changed to your info in `DNA_mapping.snakefile`. Fasta genome reference files can be indexed with `bwa index reference.fa` using BWA once `chip_zwart_public` has been activated. Genome hg38 can be downloaded [here](https://hgdownload.gi.ucsc.edu/goldenPath/hg38/bigZips/).
...
All the other parameters are already available in the `configfile_DNAmapping.yaml` file or hard-coded in the snakemake file. Inspect the hard-coded parameters to make sure they work for your data. **_NOTE_**: The default of DNA_mapping is to mark duplicates but not remove them. This is to retain true biological duplicate signal (very high ChIP-seq signal of many binding events in the same place). If you wish to remove them, add the `remove_duplicates="True"` in the DNA-mapping command (if true the tag in the bams will be *_dedup* instead of *_mdup*). 
...
**_NOTE_**: if the bam files derive from the [DNA-mapping pipeline](#dnamapping) you can save time by adding the flag `skip_bam_filtering="True"` (MAPQ filter and MarkDuplicates are skipped). You may need to add/modify the flag for the bam suffix to `bam_suffix="_mapq20_mdup_sorted.bam"` in order to match the extension of the output files of the [DNA-mapping pipeline](#dnamapping).
...
It may happen that the pipeline returns errors saying that certain python or R packages are not found even though the `chip_zwart_public` conda environment is loaded. <br>
